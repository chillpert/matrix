...
namespace MX
{
  ...
  void Assimp_Model::render(MX_SHADER shader)
  {
    for(unsigned int i = 0; i < m_meshes.size(); i++)
      m_meshes[i].render_mesh(shader);
  }

  Mesh Assimp_Model::process_mesh(aiMesh *mesh, const aiScene *scene)
  {
    // data to fill
  ...
  std::vector<Assimp_Texture> Assimp_Model::load_material_texture(aiMaterial *mat, aiTextureType type, std::string typeName)
  {
    std::vector<Assimp_Texture> textures;
