...
void initialize()
{
  ...
  MX::World::get().push(std::shared_ptr<MX::Scene>(new MX::Scene("debug")));

  MX_SCENE->push("Jupiter", "sphere.obj", "Root");
  MX_SCENE->push("Saturn", "sphere.obj", "Jupiter");
  MX_SCENE->push("Rock", "rock.obj", "Root");

  std::shared_ptr<MX::Node> jupiter = MX_SCENEGRAPH.search("Jupiter", MX_ROOT);
  ...
  std::shared_ptr<MX::Node> saturn = MX_SCENEGRAPH.search("Saturn", MX_ROOT);
  ...
  std::shared_ptr<MX::Node> rock = MX_SCENEGRAPH.search("Rock", MX_ROOT);
